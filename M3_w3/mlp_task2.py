...
model.add(Dense(units=8, activation='softmax',name='fifth'))
...
plt.savefig('accuracy' + MODEL_FNAME + '.png')
plt.close()
...
for i in ['second','third', 'fourth', 'third', 'fifth']:
    model_layer = Model(inputs=model.input, outputs=model.get_layer(i).output)
    ...
    print(features)
    if i=='second':
        plt.imshow((features.reshape(64, 32)), interpolation='none')
        plt.savefig('FeaturesImage1_' + MODEL_FNAME + i + '.jpg')
        plt.close()
        print('Done!')
    if i=='third':
        plt.imshow((features.reshape(32, 32)), interpolation='none')
        plt.savefig('FeaturesImage1_' + MODEL_FNAME + i + '.jpg')
        plt.close()
        print('Done!')

    if i == 'fourth':
        plt.imshow((features.reshape(32, 16)), interpolation='none')
        plt.savefig('FeaturesImage1_' + MODEL_FNAME + i + '.jpg')
        plt.close()
        print('Done!')
